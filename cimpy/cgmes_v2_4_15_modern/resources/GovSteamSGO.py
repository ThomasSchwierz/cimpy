...
class GovSteamSGO(TurbineGovernorDynamics):
    ...
    t1: Controller lag (T1).
    t2: Controller lead compensation (T2).
    t3: Governor lag (T3) (>0).
    t4: Delay due to steam inlet volumes associated with steam chest and inlet piping (T4).
    t5: Reheater delay including hot and cold leads (T5).
    t6: Delay due to IP-LP turbine, crossover pipes and LP end hoods (T6).
    k1: One/per unit regulation (K1).
    k2: Fraction (K2).
    k3: Fraction (K3).
    pmax: Upper power limit (Pmax).
    pmin: Lower power limit (Pmin).
    """
    ...
    pmin : int = Field(default=0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 
