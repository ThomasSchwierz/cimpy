...
class PhaseTapChangerAsymmetrical(PhaseTapChangerNonLinear):
    ...
    Describes the tap model for an asymmetrical phase shifting transformer in which the difference voltage vector adds
      to the primary side voltage. The angle between the primary side voltage and the difference voltage is named
      the winding connection angle. The phase shift depends on both the difference voltage magnitude and the winding
      connection angle.
    ...
    windingConnectionAngle: The phase angle between the in-phase winding and the out-of -phase winding used for creating
      ...
      voltage.  Setting this angle to 90 degrees is not the same as a symmemtrical
      transformer.
    """

    windingConnectionAngle : float = Field(default=0.0, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 
