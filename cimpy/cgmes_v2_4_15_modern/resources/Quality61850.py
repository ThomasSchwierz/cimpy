...
class Quality61850(Base):
    ...
    oscillatory: To prevent some overload of the communication it is sensible to detect and suppress oscillating (fast
      changing) binary inputs. If a signal changes in a defined time (tosc) twice in the same direction
      (from 0 to 1 or from 1 to 0) then oscillation is detected and the detail quality identifier
    ...
    suspect: A correlation function has detected that the value is not consitent with other values. Typically set by a
      network State Estimator.
    test: Measurement value is transmitted for test purposes.
    validity: Validity of the measurement value.
    """

    badReference : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    estimatorReplaced : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    failure : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    oldData : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    operatorBlocked : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    oscillatory : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    outOfRange : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    overFlow : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    source : Optional[str] = Field(default=None, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    suspect : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    test : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 

    validity : Optional[str] = Field(default=None, json_schema_extra={"in_profiles":[Profile.EQ, ]}) 
