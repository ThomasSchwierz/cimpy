...
class GovHydro3(TurbineGovernorDynamics):
    """
    Modified IEEE Hydro Governor-Turbine Model.  This model differs from that defined in the IEEE modeling guideline
      paper in that the limits on gate position and velocity do not permit "wind up" of the upstream signals.
    ...
    tt: Power feedback time constant (Tt).  Typical Value = 0.2.
    ...
    governorControl : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    rgate : float = Field(default=0.0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    relec : float = Field(default=0.0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 
