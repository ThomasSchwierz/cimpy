...
class GovGAST2(TurbineGovernorDynamics):
    ...
    x: Governor lead time constant (X).
    y: Governor lag time constant (Y) (>0).
    z: Governor mode (Z). true = Droop false = ISO.
    ...
    af1: Exhaust temperature Parameter (Af1).  Unit = per unit temperature.  Based on temperature in degrees C.
    bf1: (Bf1).  Bf1 = E(1-w) where E (speed sensitivity coefficient) is 0.55 to 0.65 x Tr.  Unit = per unit
    ...
    cf2: Coefficient defining fuel flow where power output is 0% (Cf2).  Synchronous but no output.  Typically 0.23 x K
      (23% fuel flow).
    tr: Rated temperature (Tr).  Unit = [SYMBOL REMOVED]C depending on parameters Af1 and Bf1.
    k6: Minimum fuel flow (K6).
    tc: Temperature control (Tc).  Unit = [SYMBOL REMOVED]F or [SYMBOL REMOVED]C depending on constants Af1 and Bf1.
    """

    mwbase : float = Field(default=0.0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    w : float = Field(default=0.0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    x : int = Field(default=0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    y : int = Field(default=0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    z : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.DY, ]}) 
