...
class Pss1A(PowerSystemStabilizerDynamics):
    ...
    inputSignalType: Type of input signal.
    a1: Notch filter parameter (A1).
    a2: Notch filter parameter (A2).
    t1: Lead/lag time constant (T1).
    t2: Lead/lag time constant (T2).
    t3: Lead/lag time constant (T3).
    t4: Lead/lag time constant (T4).
    t5: Washout time constant (T5).
    t6: Transducer time constant (T6).
    ks: Stabilizer gain (Ks).
    vrmax: Maximum stabilizer output (Vrmax).
    vrmin: Minimum stabilizer output (Vrmin).
    ...
    kd: Selector (Kd).  true = e used false = e not used.
    tdelay: Time constant (Tdelay).
    """
    ...
    kd : bool = Field(default=False, json_schema_extra={"in_profiles":[Profile.DY, ]}) 

    tdelay : int = Field(default=0, json_schema_extra={"in_profiles":[Profile.DY, ]}) 
